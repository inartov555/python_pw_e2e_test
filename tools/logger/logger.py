...
class Logger:
    ...
    def __update_handler(self, logr, handlr):
        """
        Update the logger's handler with the new log level. The method will get all handlers of the logger and change the level
    ...
    def setup_filehandler(self, file_name: str, level: str = "DEBUG"):
        """
        Method to setup file handler for a particular logger or all available loggers
        If the file handler was set up for all loggers, then all new ones will be created with the same config
